...
public:
    vector<double> medianSlidingWindow(vector<int>& a, int k) {
        vector<double> ans;
        int n = a.size();
        OrderedSet<pair<double,int>> st;

        function<double()> getMedian = [&] ()
        {
            if(k&1)
            {
                return st.getKthElement(k/2).first;
            }
            else
            {
                return (st.getKthElement(k/2).first + st.getKthElement(k/2-1).first)/2.0;
            }
        };
